        ...
        public virtual bool CheckInsert(int id, VariableInventorySystem.IVariableInventoryCellData cellData)
        {
            ...
            for (var i = 0; i < width; i++)
            {
                for (var t = 0; t < height; t++)
                {
                    ...
                    {
                        if (CellData[id + i + (t * CapacityWidth)] != null)
                        {
                            if (CellData[id + i + (t * CapacityWidth)].IsStackable && cellData.IsStackable && CellData[id + i + (t * CapacityWidth)].type == cellData.type
                                                        && CellData[id + i + (t * CapacityWidth)].maxAmount >= CellData[id + i + (t * CapacityWidth)].currentAmount)
                            {
                                return true;
                            }
                        }
