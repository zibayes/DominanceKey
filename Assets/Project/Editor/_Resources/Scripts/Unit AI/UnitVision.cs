...
public class UnitVision : MonoBehaviour
{
    ...
    bool GetRaycast(Vector3 dir)
    {
        ...
        if (Physics.Raycast(pos, dir, out hit, distance))
        {
            ...
            if (obj != null)
            {
                if (playerController.selectManager.selectableChars.Contains(obj.selection))
                {
                    if (playerController.selection.player != obj.selection.player)
                    {
                        result = true;
    ...
    void Update()
    {
        //if (Vector3.Distance(transform.position, target.position) < distance) // Вариант оптимизации...
        {
            ...
            else
            {
                ...
                if (currentTarget == null)
                {
                    currentTarget = playerController.noiseDetectionTarget;
                }
                else if(!Input.GetKey(KeyCode.LeftControl) && !playerController.UIController.isActiveManualControl)
                {
